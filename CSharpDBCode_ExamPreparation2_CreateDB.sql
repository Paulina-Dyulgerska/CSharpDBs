...
--da pochvam ot tablici, ot koito nishto ne izliza natatyk:

--	Username nvarchar(30) UNIQUE NOT NULL, --DEFAULT 'None' ili INDEX i t.n.
----UNIQUE pravi taka che avtomatichno se syzdava INDEX za unikalnite neshta v kolonata!!! No UNIQUE NE pravi constraint!!!
----UNIQUE pravi index i az moga da go vidq w papkata na tablicata s indexite j!!! Tam se vijda, che UNIQUE syzdawa non-clustered Index!!!
----UNIQUE shte me ogranichi da vkaram povtarqshti se imena. No Judge moje i da ne go haresa, ako tyrsi CONSTRAINT!!!
--	CHECK (LEN(Username) > 3 AND Username IS NOT NULL) --tova e check constraint
...
(
	...
	[Password] nvarchar(50) NOT NULL, --ako okolo Password imashe neshto, s koeto da si napravi
	--izpylnima kombinaciq ot komandi za SQL, to shteshe da mi dade error tuk.
	--da wnimawam i ako imam error pokraj ime ocveteno v sinyo, da znam
	--che trqbwa da eskeipna imeto s [] i pak da probwam!!!
	[Name] nvarchar(50),
	Birthdate datetime, --datetime2 e po-dobyr type ot datetime!!!! Ako e realna DB, da polzwam datetime2!!! Entity Framework 
	--gi pravi avtomatichno datetime2 datite!!!
...
--createvam si tables i si pravq diagramata na DB-a.
--insertvam si neshtata, koito sa mi dadeni v DDL Dataset-a!!!!
USE Service
GO
...
--razglejdam si tablicite i prodyljavam s insertite i selectite v nov file!!! 
--S tozi shte si runna DB-a, ako go sbozqsam!!!
